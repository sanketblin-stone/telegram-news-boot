...
async def news_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = "📰 <b>Today's Smart News Digest</b>\n\n"
    for item in news_items:
        safe_title = html.escape(item["title"])
        safe_category = html.escape(item["category"])
        message += f"<b>{safe_category}:</b> {safe_title}\n{item['link_type']} {item['link']}\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not search_results:
        ...
        message = f"🔍 <b>Search Results for '{text}'</b>\n\n"
...
async def daily_digest(context: ContextTypes.DEFAULT_TYPE):
    ...
    message = "📰 <b>Your Phase 2.5 Daily Digest</b>\n\n"
    for item in news_items:
        safe_title = html.escape(item["title"])
        safe_category = html.escape(item["category"])
        message += f"<b>{safe_category}:</b> {safe_title}\n{item['link_type']} {item['link']}\n\n"
    ...
    if len(message) > 4000:
        for i in range(0, len(message), 4000):
            await context.bot.send_message(
                chat_id=MY_CHAT_ID, text=message[i : i + 4000], parse_mode="HTML"
            )
    else:
        await context.bot.send_message(
            chat_id=MY_CHAT_ID, text=message, parse_mode="HTML"
        )
